...
QVector<QPoint> MainWindow::BresenhamLine(double x0, double y0, double x1, double y1){
    ...
    //отражаем линию по диагонали если угол слишком большой
    if(steep){
        std::swap(x0,y0);
        std::swap(x1,y1);
    }
    //если линия растет не слева направо то меняем начало и конец
    if(x0>x1){
        std::swap(x0,x1);
        std::swap(y0,y1);
    }
    int dx = x1-x0;
    int dy = abs(y1-y0);
    int error = dx/2;//используем оптимизацию умножения на dx чтобы избавиться от лишних дробей
    int ystep = (y0<y1) ? 1:-1;//выбираем направление роста координаты y
    int y = y0;

    QVector<QPoint> line;
    for(int x = x0; x<= x1;x++){
        line.push_back(steep?QPoint(y,x):QPoint(x,y));//возвращаем координаты на место если меняли
        error -= dy;
        if(error<0){
            y+=ystep;
            error+=dx;
